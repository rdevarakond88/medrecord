...
## Current Status
**Phase:** BUG-D3-DT5-1 FIXED (2026-03-29) — sync worker stale-ref bug resolved; visit sync now works on fresh-login sessions. D3 device test session 6 required to verify fix + re-verify BUG-D3-DT1-2 cross-session persistence.
...
---
**Last Session:** Builder (2026-03-29). BUG-D3-DT5-1 FIXED — stale doctorIdRef in useSyncWorker caused sync worker to query sync_queue with doctor_id='' on fresh-login sessions; visits never found. Fix: runSyncWorker now reads doctorId directly from useAuthStore at call time. Secondary fix: dead BASE_URL in syncWorker.ts tryRefreshToken replaced with exported API_BASE_URL from apiClient.ts.

### Recommended Next Session Order
| Priority | Session | Reason |
|---|---|---|
| 1 | **D3 device test session 6 (Step 8)** | Verify BUG-D3-DT5-1 fix (visits now sync), re-verify BUG-D3-DT1-2 cross-session persistence |
