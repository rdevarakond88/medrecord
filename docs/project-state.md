...
Standing history (not this session's routing reason): PR #7 merged dev → main
        ...
        Hardening backlog: #1, #2, #3, #4, #7, #9, #10 CLOSED. #5 (document-only,
        no fix planned) and #8 (agent-gate.sh cosmetic regex false-positive, low
        priority) remain OPEN, non-blocking. #6 remains OPEN and is the one
