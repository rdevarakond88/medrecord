...
**Last Session:** D2 real device verification (2026-02-22). Five items confirmed on real iPhone via Expo Go: search bar blue border on tap, unfocus on outside tap, blinking cursor after last typed digit, FAB not overlapping keypad key 3, digit entry via keypad. Fixes committed 14b6894 + 5aa5ff1. Validation checklist saved to `reviews/D2-VALIDATION-CHECKLIST.md`. Only H-2 (certificate pinning) and H-3 (offline audit log) remain open — both are pre-v1 merge-to-main blockers, not D3 blockers. **Next: D3 mockup.**
...
| ~~**H-4:** No auth guard on D2 mount; `getRecentPatients` runs before `token` is confirmed non-null~~ | D2 | Security audit L-3 / QA H-4 | **CLOSED** — `useEffect` on `[token, user]` calls `navigation.replace('Login')` if either is falsy; both `getRecentPatients` and `searchPatientsByMobile` effects guarded with `if (!token \|\| !user) return`. |
