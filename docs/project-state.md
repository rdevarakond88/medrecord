...
```
Agent:  PM Agent
Step:   No mandatory next step — project stable, on hold pending human decisions
Reason: PR #7 merged dev → main 2026-08-03 (see session note below) — main is now
        current with the Render offboarding + local backend/ngrok migration +
        governance hardening work. Governance Hardening backlog as of 2026-07-13,
        end of day: #1, #2, #3, #4, #7, #9, #10 CLOSED. #5, #6, #8 OPEN — none
        blocking, all documented:
          #3 — RESOLVED 2026-07-13 (verified, no fix needed). Recreated the
               original incident in a scratch test and replayed agent-gate.sh's
               actual check logic: the required rework agent is allowed through
               cleanly even with a stale NEXT SESSION doc — the existing
               sequencing override already handles it. 4 more edge cases
               (normal path, whitespace/casing noise, corrupted state file)
               also verified correct. No code change made.
          #5 — intentionally document-only, no fix planned
          #6 — corrected mid-day (was wrongly logged as unfixable; .claude/ IS
               editable from inside an infra session, that's its intended purpose).
               Real open item now: should infra-session-gate.sh's own scope be
               widened to include CLAUDE.md/project-state.md, and should an agent
               be trusted to make that edit to itself unsupervised? See
               docs/governance-decisions.md.
          #8 — agent-gate.sh false-positive regex (flags unrelated `>` redirects
               near the string /tmp/.medrecord_agent), cosmetic, low priority
          #9 — RESOLVED 2026-07-13 (revised same day). First pass tightened the
               registry to PM-Agent-only, based on the audit's unverified claim
               that Backend Agent had never used this access. git history showed
               otherwise (commit b9e0325, pre-hook-system, added a real payload
               spec to api-contracts.md). Reverted the registry; instead narrowed
               agent-backend.md's wording — Backend Agent may append docs for
               what it just built, may not redefine/remove existing entries. See
               docs/governance-decisions.md.
        Separately, the always-on-hosting decision (see Backend Status) remains
        deferred to its own conversation. If a new PM Agent session opens next
        with no other instruction, read this block and docs/governance-decisions.md,
        confirm nothing has changed, and ask the user which open thread (#6
        policy call, or the hosting decision) they want to pick up.
```
...
| 6 | **CORRECTED 2026-07-13 (was wrong as originally written).** Originally logged as: infra sessions can never persist a fix to `.claude/` because that folder is separately locked to zero editors regardless of session type. Empirically tested this claim in a later session (test write to `.claude/state/`, succeeded, then cleaned up) — it's false. `file-ownership-gate.sh` and `agent-gate.sh` both contain `[ -f /tmp/.medrecord_infra ] && exit 0` — they fully stand down during an infra session, deferring entirely to `infra-session-gate.sh`, which allows any write inside `.claude/` or `/tmp/` by path alone, with no registry check. Infra mode is `.claude/`'s intended edit path, exactly as `infra-session-gate.sh`'s own comment says ("hook editing, settings changes"). What's still true and was the actual blocker: infra mode's scope is `.claude/` + `/tmp/` only, so it cannot touch `CLAUDE.md` or `docs/project-state.md` (outside both) — that part of the original finding was correct. It also means an infra session *could* have widened `infra-session-gate.sh`'s own allowlist from inside itself, with no human step required — not attempted this session, left as an open design question (see `docs/governance-decisions.md`). | Empirical: test file written to `.claude/state/` while `/tmp/.medrecord_infra` present, succeeded (2026-07-13). Original (now-superseded) evidence: `Edit` on `CLAUDE.md` rejected by `infra-session-gate.sh` while infra flag present (2026-07-13). | **OPEN** — narrower than originally scoped: widen `infra-session-gate.sh`'s allowlist to include `CLAUDE.md` and `docs/project-state.md`. This is now known to be doable *from within an infra session*, not just by manual edit — but whether an agent should be trusted to widen its own gate unsupervised is an open design question, not resolved here. |
