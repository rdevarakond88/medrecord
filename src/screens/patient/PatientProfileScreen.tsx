 ...
 *
 * Wire step will:
 *   1. Load patient profile from GET /patient/profile (id, name, mobile, dob, language).
 *   2. Save edits via PATCH /patient/profile (name, dob, language).
 *   3. Persist language preference to AsyncStorage/SecureStore for app-wide use.
 *   4. Logout: clear patient auth state + expo-secure-store refresh token,
...
type DemoState = 'viewing' | 'editing';
...
export default function PatientProfileScreen() {
  ...
  const [draftDob,       setDraftDob]       = useState(MOCK_PROFILE.dob);
  const [draftLanguage,  setDraftLanguage]  = useState<Language>(MOCK_PROFILE.language);
  const [langModalOpen,  setLangModalOpen]  = useState(false);

  const [demoState, setDemoState] = useState<DemoState>('viewing');
...
const styles = StyleSheet.create({
  ...
  langOptionSelected: {
    // no background change — checkmark is the indicator
  },
  langOptionText: {
    fontSize:   17,
    color:      Colors.textPrimary,
